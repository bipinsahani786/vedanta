...
class ReferralController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        $successfulReferrals = Referral::where('stage', 'placed')->count();
        $pendingReferrals = Referral::where('stage', '!=', 'placed')->where('status', 'active')->count();
        ...
        $thisMonthSuccessful = Referral::where('stage', 'placed')->whereBetween('updated_at', [$startDate, $endDate])->count();
        ...
        $funnel = [
            ...
            'profile_completed' => Referral::whereIn('stage', ['profile_completed', 'verified', 'interview_scheduled', 'placed'])->count(),
            'interview_scheduled' => Referral::whereIn('stage', ['interview_scheduled', 'placed'])->count(),
            'placed' => Referral::where('stage', 'placed')->count(),
        ];
        ...
        for ($i = 6; $i >= 0; $i--) {
            ...
            $growthSuccessful[] = Referral::where('stage', 'placed')->where('created_at', '<=', $dayEnd)->count();
            $growthPending[] = Referral::where('stage', '!=', 'placed')->where('created_at', '<=', $dayEnd)->count();
        }
        ...
        $topReferrers = User::whereHas('referralsMade')
            ->withCount(['referralsMade as total_referred', 'referralsMade as placed_count' => function ($q) {
                $q->where('stage', 'placed');
            }])
            ->with('referralWallet')
