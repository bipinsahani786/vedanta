...
@section('actions')
    <a href="{{ route('admin.locations.create') }}" class="px-4 py-2 bg-accent-blue text-white rounded-xl text-sm font-semibold hover:bg-accent-blue-hover transition-all shadow-lg flex items-center gap-2">
...
<div class="bg-card-bg rounded-t-2xl border-x border-t border-card-border p-4 flex flex-col sm:flex-row justify-between items-center gap-4">
    ...
    <form action="{{ route('admin.locations.index') }}" method="GET" class="w-full sm:w-auto flex items-center relative">
        <i class="fas fa-search absolute left-3 text-text-dark/40 text-sm"></i>
        <input type="text" name="search" value="{{ request('search') }}" placeholder="Search city or state..." 
        ...
        @if(request('search'))
            <a href="{{ route('admin.locations.index') }}" class="absolute right-3 text-text-dark/40 hover:text-red-400 transition-colors">
    ...
    <table class="w-full text-left border-collapse admin-table">
        <thead>
            <tr>
                ...
                <th>
                    <a href="{{ route($route, array_merge(request()->query(), ['sort_by' => 'city', 'order' => $order])) }}" class="flex items-center gap-2 hover:text-accent-blue transition-colors">
                        City
                ...
                <th>
                    <a href="{{ route($route, array_merge(request()->query(), ['sort_by' => 'state', 'order' => $order])) }}" class="flex items-center gap-2 hover:text-accent-blue transition-colors">
                        State
                        @if(request('sort_by') === 'state')
                            <i class="fas fa-sort-{{ request('order') === 'asc' ? 'up' : 'down' }} text-accent-blue"></i>
                        @else
                            <i class="fas fa-sort text-text-dark/20"></i>
                        @endif
        ...
        <tbody class="divide-y divide-card-border">
            ...
            <tr>
                <td class="font-medium text-text-dark/50">#{{ $location->id }}</td>
                <td class="font-semibold text-text-main">{{ $location->city }}</td>
                <td class="text-text-main">{{ $location->state }}</td>
                <td>
                ...
                <td class="text-text-dark/60">{{ $location->created_at->format('M d, Y') }}</td>
                <td>
                    <div class="flex items-center justify-end gap-2">
                        <a href="{{ route('admin.locations.edit', $location) }}" class="w-8 h-8 rounded-lg bg-accent-blue/10 text-accent-blue flex items-center justify-center hover:bg-accent-blue hover:text-white transition-colors tooltip" title="Edit">
            ...
            <tr>
                <td colspan="6" class="py-12 text-center">
                    ...
                    <p class="text-text-main font-semibold mb-1">No locations found</p>
                    <p class="text-text-dark/40 text-sm">Try adjusting your search criteria or add a new location.</p>
                </td>
            </tr>
            @endforelse
        </tbody>
