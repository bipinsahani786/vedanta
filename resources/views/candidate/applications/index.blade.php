        ...
        <div class="overflow-x-auto">
            <table class="w-full text-left border-collapse">
                ...
                <tbody class="text-sm divide-y divide-card-border" x-data="{ expandedId: null }">
                    ...
                    <tr class="hover:bg-secondary-bg/30 transition-colors cursor-pointer group" @click="expandedId === {{ $app->id }} ? expandedId = null : expandedId = {{ $app->id }}">
                        <td class="px-6 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-10 h-10 bg-accent-blue/10 rounded-xl flex items-center justify-center text-accent-blue text-sm font-bold shrink-0 group-hover:scale-110 transition-transform">
                        ...
                        <td class="px-6 py-4">
                            ...
                            @elseif($app->status === 'shortlisted')
                                <span class="inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-semibold bg-accent-yellow/10 text-accent-yellow border border-accent-yellow/20">
                                    <i class="fas fa-share mr-1.5 text-[9px]"></i> Forwarded
                                </span>
                    ...
                    <tr x-show="expandedId === {{ $app->id }}" class="bg-secondary-bg/20" x-transition.opacity style="display: none;">
                        <td colspan="4" class="px-6 py-6 border-b-2 border-accent-blue/30">
                            <div class="p-5 border border-card-border rounded-2xl bg-card-bg shadow-inner">
                                ...
                                <div class="relative flex flex-col md:flex-row justify-between w-full mb-6 gap-6 md:gap-0">
                                    <!-- Connecting Line -->
                                    <div class="absolute top-4 left-[10%] w-[80%] h-1 bg-card-border z-0 hidden md:block">
                                    ...
                                    <div class="relative z-10 flex flex-col items-center flex-1">
                                        <div class="w-8 h-8 rounded-full bg-green-500 text-white flex items-center justify-center shadow-[0_0_15px_rgba(34,197,94,0.4)] border-2 border-card-bg z-10">
                                            <i class="fas fa-check text-xs"></i>
                                        </div>
                                        <div class="mt-3 text-sm font-bold text-text-main">Applied</div>
                                        <div class="text-[10px] text-text-dark/50">{{ $app->created_at->format('d M, Y h:i A') }}</div>
                                    </div>

                                    <!-- Step 2: Forwarded -->
                                    <div class="relative z-10 flex flex-col items-center flex-1">
                                        <div class="w-8 h-8 rounded-full flex items-center justify-center border-2 border-card-bg z-10 transition-colors duration-300 {{ in_array($app->status, ['shortlisted', 'hired', 'rejected']) ? 'bg-accent-yellow text-[#031b4e] shadow-[0_0_15px_rgba(255,184,0,0.4)]' : 'bg-card-border text-text-dark/40' }}">
                                            <i class="fas {{ in_array($app->status, ['shortlisted', 'hired', 'rejected']) ? 'fa-check' : 'fa-hourglass-half' }} text-xs"></i>
                                        </div>
                                        <div class="mt-3 text-sm font-bold {{ in_array($app->status, ['shortlisted', 'hired', 'rejected']) ? 'text-text-main' : 'text-text-dark/50' }}">Forwarded to School</div>
                                        @if($app->is_forwarded)
                                        <div class="text-[10px] text-text-dark/50"><i class="fas fa-share text-[8px]"></i> Profile sent to employer</div>
                                        @else
                                    ...
                                    <div class="relative z-10 flex flex-col items-center flex-1">
                                        <div class="w-8 h-8 rounded-full flex items-center justify-center border-2 border-card-bg z-10 transition-colors duration-300 
                                        ...
                                        <div class="text-[10px] {{ in_array($app->status, ['hired', 'rejected']) ? 'text-text-dark/50' : 'text-text-dark/40' }}">
                                ...
                                @if($app->remarks)
                                <div class="mt-6 p-4 bg-accent-blue/5 border border-accent-blue/20 rounded-xl relative overflow-hidden">
                                    <div class="absolute -right-4 -top-4 text-accent-blue/10 text-5xl"><i class="fas fa-quote-right"></i></div>
                                    <h5 class="text-xs font-bold text-accent-blue uppercase tracking-wider mb-2 flex items-center gap-2">
                                        <i class="fas fa-comment-dots"></i> Update / Remarks:
                                    </h5>
                                    <p class="text-sm text-text-main relative z-10 leading-relaxed">{{ $app->remarks }}</p>
                                </div>
                                @elseif($app->status === 'shortlisted')
