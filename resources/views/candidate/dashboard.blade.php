        ...
        <div class="bg-gradient-to-r from-accent-blue to-accent-blue-hover rounded-3xl p-8 mb-8 text-white shadow-lg relative overflow-hidden reveal">
            ...
            <div class="relative z-10 flex flex-col md:flex-row items-center gap-6">
                @if($profile->profile_photo_path)
                    <img src="{{ asset('storage/' . $profile->profile_photo_path) }}" alt="Profile Photo" class="w-24 h-24 rounded-full object-cover border-4 border-white/20 shadow-xl">
                @else
                    <div class="w-24 h-24 rounded-full bg-white/20 flex items-center justify-center text-4xl border-4 border-white/20 shadow-xl">
                ...
                <div class="text-center md:text-left flex-1">
                    <h1 class="text-3xl font-bold mb-1 flex items-center flex-wrap gap-2">
                        ...
                        @if($profile->is_verified)
                            <span class="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-500/20 border border-blue-400/50 text-blue-300 text-xs font-bold uppercase tracking-wider rounded-full shadow-[0_0_15px_rgba(59,130,246,0.3)]" title="Verified Profile">
                ...
                <div class="mt-4 md:mt-0 flex gap-3">
                    <a href="{{ route('jobs') }}" class="px-6 py-3 bg-white text-accent-blue font-bold rounded-xl hover:bg-gray-50 transition-all shadow-md flex items-center gap-2">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-6 reveal reveal-delay-1">
                    <div class="bg-card-bg rounded-2xl border border-card-border p-6 flex flex-col items-center justify-center text-center hover:border-accent-blue/30 transition-all shadow-sm relative">
                    ...
                    <div class="bg-card-bg rounded-2xl border border-card-border p-6 flex flex-col items-center justify-center text-center hover:border-green-500/30 transition-all shadow-sm">
                        <div class="w-12 h-12 rounded-xl bg-green-500/10 text-green-400 flex items-center justify-center text-xl mb-3">
                            <i class="fas fa-check-double"></i>
                        </div>
                        <h3 class="text-3xl font-bold text-text-main">{{ auth()->user()->applications()->where('status', 'shortlisted')->count() }}</h3>
                        <p class="text-xs font-semibold text-text-dark/50 uppercase tracking-wide mt-1">Shortlisted</p>
                    </div>
                    <div class="bg-card-bg rounded-2xl border border-card-border p-6 flex flex-col items-center justify-center text-center hover:border-accent-yellow/30 transition-all shadow-sm">
                        <div class="w-12 h-12 rounded-xl bg-accent-yellow/10 text-accent-yellow flex items-center justify-center text-xl mb-3">
                            <i class="fas fa-briefcase"></i>
                        </div>
                        <h3 class="text-3xl font-bold text-text-main">{{ \App\Models\JobPost::where('status', 'approved')->count() }}</h3>
                ...
                <div class="bg-red-50 border border-red-200 rounded-2xl p-6 flex items-center justify-between shadow-sm reveal reveal-delay-2">
                    <div>
                        <h3 class="text-lg font-bold text-red-700 flex items-center gap-2"><i class="fas fa-exclamation-triangle"></i> Pending Amount Due</h3>
                        <p class="text-sm text-red-600/80 mt-1">You have a pending balance of ₹{{ number_format($profile->pending_amount, 2) }}. Please clear this to avoid service interruption.</p>
                    </div>
                    <a href="#" class="px-6 py-3 bg-red-600 text-white font-bold rounded-xl shadow-md hover:bg-red-700 transition-colors shrink-0">Pay ₹{{ number_format($profile->pending_amount, 0) }}</a>
                </div>
                @endif
                ...
                <div class="bg-card-bg rounded-2xl border border-card-border overflow-hidden shadow-sm reveal reveal-delay-2">
                    <div class="px-6 py-4 border-b border-card-border flex justify-between items-center bg-secondary-bg/30">
                        <h3 class="font-bold text-text-main flex items-center gap-2"><i class="fas fa-bell text-accent-yellow"></i> Notifications & Updates</h3>
                    </div>
                    <div class="divide-y divide-card-border">
                        <div class="p-5 flex gap-4 hover:bg-secondary-bg/30 transition-colors">
                            <div class="w-10 h-10 rounded-full bg-green-500/10 text-green-400 flex items-center justify-center flex-shrink-0 mt-1">
                                <i class="fas fa-check"></i>
                            </div>
                            <div>
                                <h4 class="text-sm font-bold text-text-main mb-1">Registration Complete</h4>
                                <p class="text-xs text-text-dark/70 leading-relaxed">Your profile, agreement, and payment have been verified. You can now apply to unlimited jobs.</p>
                                <span class="text-[10px] text-text-dark/40 font-medium mt-2 block">{{ $profile->updated_at->diffForHumans() }}</span>
                            </div>
                        </div>
                        <div class="p-5 flex gap-4 hover:bg-secondary-bg/30 transition-colors">
                            ...
                            <div>
                                ...
                                <p class="text-xs text-text-dark/70 leading-relaxed">We have added new teaching opportunities that match your preferred location and subject.</p>
                    ...
                    <div class="px-6 py-3 border-t border-card-border bg-secondary-bg/30 text-center">
                        <a href="#" class="text-xs font-semibold text-accent-blue hover:text-accent-blue-hover">View All Notifications</a>
                    </div>
                </div>
            ...
            <div class="space-y-6 reveal reveal-delay-3">
                <div class="bg-card-bg rounded-2xl border border-card-border overflow-hidden shadow-sm relative">
                    <div class="px-6 py-5 border-b border-card-border {{ $profile->plan_type === 'premium' ? 'bg-gradient-to-r from-accent-yellow/20 to-transparent border-accent-yellow/30' : 'bg-secondary-bg/30' }}">
                        <div class="flex justify-between items-start mb-2">
                            <h3 class="font-bold text-text-main flex items-center gap-2">
                                <i class="fas fa-star {{ $profile->plan_type === 'premium' ? 'text-accent-yellow' : 'text-text-dark/40' }}"></i> Current Plan
                            </h3>
                            <span class="px-3 py-1 bg-green-500/10 text-green-400 text-[10px] font-bold uppercase tracking-wider rounded-lg">Active</span>
                        </div>
                        <div class="mt-4">
                            <span class="text-3xl font-black text-text-main capitalize">{{ $profile->plan_type ?? 'Standard' }}</span>
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        ...
                        @if($profile->plan_type !== 'premium')
                            <div class="pt-4 border-t border-card-border">
                                <p class="text-xs text-text-dark/60 mb-3 text-center">Get more opportunities and faster placements with Premium.</p>
                                <a href="{{ route('candidate.payment.show') }}" class="block w-full py-3 bg-gradient-to-r from-accent-yellow to-yellow-500 text-[#031b4e] font-bold text-sm text-center rounded-xl hover:shadow-lg hover:-translate-y-0.5 transition-all">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            ...
            <div class="bg-card-bg rounded-2xl border transition-all duration-300 hover:shadow-xl group overflow-hidden reveal reveal-delay-1
                ...
                <div class="p-6 flex flex-col items-center text-center">
                    ...
                    @else
                        <a href="{{ route('candidate.wizard') }}" class="mt-auto w-full px-4 py-3 rounded-xl text-sm font-semibold bg-accent-blue text-white hover:bg-accent-blue-hover hover:-translate-y-0.5 shadow-lg transition-all flex items-center justify-center gap-2">
            ...
            <div class="bg-card-bg rounded-2xl border transition-all duration-300 hover:shadow-xl group overflow-hidden reveal reveal-delay-2
                ...
                <div class="p-6 flex flex-col items-center text-center">
                    ...
                    @if($profile->is_agreement_signed)
                        <a href="{{ route('candidate.agreement.show') }}" class="mt-auto w-full px-4 py-3 rounded-xl text-sm font-semibold border border-green-500/20 text-green-400 hover:bg-green-500/5 transition-all flex items-center justify-center gap-2">
            ...
            <div class="bg-card-bg rounded-2xl border transition-all duration-300 hover:shadow-xl group overflow-hidden reveal reveal-delay-3
                ...
                <div class="px-6 pt-5 pb-0 flex items-center justify-between">
                    <span class="text-[10px] font-bold uppercase tracking-widest {{ $profile->initial_fee_paid ? 'text-green-400' : ($profile->is_agreement_signed ? 'text-accent-yellow' : 'text-text-dark/30') }}">Step 3</span>
